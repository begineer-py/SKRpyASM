...
- **[SKRpyASM_技術白皮書.md](SKRpyASM_技術白皮書.md)** - 完整的技术白皮书（主文档，4000+ 行）
  - 系统概述与架构
  - 三层 AI Agent 架构详解
...
- **[docs/](docs/)** - 详细文档目录
  - [构建指南](docs/BUILD_GUIDE.md) - 环境配置与部署
  - [CVE API 指南](docs/CVE_API_GUIDE.md) - CVE Intelligence REST API 使用
  - [CVE 实现总结](docs/CVE_IMPLEMENTATION_SUMMARY.md) - CVE 系统架构
...
- **AI-assisted analysis / AI 輔助分析**: run initial asset triage, convert high-value findings into `Overview` records, and generate planning context for automation.
- **Scheduler and automation / 排程與自動化**: orchestrate recurring scan, fetch, and watchdog jobs through Celery and django-celery-beat, then hand execution to the internal automation framework.
...
│   ├── api_keys/         # API keys, tool config download, and Agent LLM config management
│   └── http_sender/      # Endpoint fuzzing helpers built around ffuf and Hasura metadata
