...
- 持有系統主要模型
- 提供 `ExecutionGraph`/`ExecutionNode` 與 `Overview` 的對外 API
- 承載 assistant/thread/message、compression、analysis、ThreadEvent 等核心資料結構
...
- 分析模型：`InitialAIAnalysis`、`Overview`、`AttackVector`
- 執行監控模型：`ExecutionGraph`、`ExecutionNode`、`ExecutionEvent`、`ExecutionArtifact`（取代舊 `Step`/`StepLog`/`ScriptExecution`）
...
來源：`apps/core/execution_api.py`

- `GET /api/core/executions/` — 查詢 ExecutionGraph（支援 `target_id`、`thread_id`、`status` 過濾）
- `GET /api/core/executions/{execution_id}` — 細節含 nodes/events/artifacts
- `POST /api/core/executions/` — 建立新的 ExecutionGraph
...
- 更準確的說法是：
  - `apps/core` 是核心模型層
  - 對外 API 目前主要集中在 `ExecutionGraph` 和 `Overview`
...
- `apps/auto` 以 `Overview`、`ExecutionNode`、`Thread` 為核心執行自動化
