...
class CyberDataFactory:
    ...
    def _process_and_write(self, raw_res: str, batch_nodes: List[Dict]):
        try:
            ...
            inner_text = outer["choices"][0]["message"]["content"]
    ...
    def run(self):
        ...
        for d in range(curr_d, -1, -1):
            ...
            for i in range(start_i, total, self.batch_size):
                ...
                try:
                    res = requests.post(
                        self.nyaproxy_url, json=_build_payload(full_prompt), timeout=120
                    )
...
if __name__ == "__main__":
    factory = CyberDataFactory(input_file="target_ck.json", batch_size=30)
    factory.run()
