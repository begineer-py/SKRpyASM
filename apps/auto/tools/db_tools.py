...
class DBToolsMixin:
    ...
    def log_step_action(
        ...
        """
        📍 P2.2 FIX: 記錄 Step 執行的動作日誌。
        
        在執行任何操作前呼叫此工具，記錄即將進行的動作。
        
        Args:
            step_id: 所屬 Step 的 ID
            action_type: 動作類型 (SKILL_EXEC, COMMAND, API_CALL, SCAN 等)
            message: 詳細的動作描述 (例如: "執行 Skill test_django --url=target.com")
            tag: 日誌分類標籤
        ...
        """
        try:
            from apps.core.models import StepLog, Step
    ...
    def log_step_result(
        ...
        """
        📍 P2.2 FIX: 記錄 Step 執行結果。
        
        在完成動作後呼叫此工具，記錄執行結果（成功或失敗）。
        
        Args:
            step_id: 所屬 Step 的 ID
            status: 結果狀態 (SUCCESS, FAILED, PARTIAL, SKIPPED)
            content: 詳細的結果內容
            execution_time_ms: 執行耗時（毫秒）
            level: 日誌等級 (RESULT, WARN, ERROR)
    ...
    def log_ai_thought(
        ...
        """
        📍 P2.2 FIX: 記錄 AI 的思考過程。
        
        當 AI 分析結果、做出決策時呼叫此工具，記錄推理邏輯。
        這對後續的調試和學習非常重要。
