...
Content is the primary interface. Layout, controls, and supporting context should make the current work easier to read before they expose every available option.

- **Hierarchy**: Put the active conversation, result, or data set at the center of the workspace. Use the existing page and section title levels only when they add orientation, then place supporting metadata at `text-sm` or `text-xs font-mono` according to the Typography rules. Do not introduce a decorative kicker, repeated route name, or unverified operational claim.
- **Readable text**: Keep conversation text, summaries, instructions, and other prose in a readable content column rather than stretching paragraphs across the viewport. Use `font-body`, `text-base` for primary reading, and `text-sm` only for supporting content. The surrounding workspace may remain wide, but the reading measure must stay visually distinct from data-heavy regions.
- **Wide data canvases**: Execution graphs, event tables, asset results, and other structured data may use an intentionally wide canvas. Preserve the existing `p-6` and `md:p-8` workspace gutters, use tonal surfaces for separation, and keep dense values in `font-mono` without shrinking body content below `text-sm`. A wide canvas supports scanning data; it must not make prose or primary actions span the viewport.
- **One focal action**: Each content surface has one visually primary next action, such as starting a conversation, opening a Target, or opening an execution in the monitor. Use the established primary control treatment and give secondary, destructive, filtering, and configuration actions a separate labelled menu, popover, or Drawer. Do not make every available action equally prominent.
- **Contextual disclosure**: Low-frequency controls, agent details, thread events, filters, settings, topology context, and diagnostic detail stay closed until requested. Reuse the existing Drawer for focused secondary workflows and the existing Dialog or popover guidance for smaller contextual groups. Every disclosed surface needs a meaningful title or label, `aria-expanded` and `aria-controls` when a trigger owns a region, keyboard activation, Escape close, visible focus, and focus restoration.
- **Target context**: Target remains the primary product noun. Group conversations, summaries, assets, and execution links by Target where that relationship exists. Use “unbound” only for a conversation without a Target. Do not introduce a second ownership noun or imply a new ownership model.
- **Compact app rail**: The shared navigation uses a compact app rail on larger screens and a mobile Drawer at smaller widths. Keep every existing route reachable exactly once, show the active route with the existing cyan interaction treatment, and provide an accessible name, visible focus ring, and predictable keyboard behavior for each icon. The rail is navigation, not a second content surface, so low-frequency settings remain in contextual disclosure.
- **AI contextual drawer**: AI Center reuses the shared Drawer pattern for agent tree, thread events, topology or asset context, overview, filters, and settings. Keep chat and result summaries in the main readable surface. The Drawer uses the existing `--bg-elevated`, `rounded-2xl`, `p-6`, `text-xl` title, `text-sm` description, and Radix focus behavior. Execution detail opens the canonical monitor rather than being duplicated inline.
- **Reusable surface checklist**: For every new AI, rail, or monitor surface, identify the content it serves, its single focal action, its disclosure trigger, and its empty or error recovery. Apply existing `--bg-base`, `--bg-card`, `--bg-surface`, `--bg-elevated`, `--text-*`, `--border-*`, and `--cyan` roles, the Typography scale above, and only the documented 4px spacing steps. Do not add raw color, spacing, radius, or shadow values to create a one-off variant.
