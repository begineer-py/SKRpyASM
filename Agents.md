...
- Prefer small, bounded agent tasks that change or verify one focused concern at a time. Avoid delegating large end-to-end implementation or review tasks when the work can be split into short, independently verifiable steps.
- The coordinating agent may edit code directly for local, well-understood fixes; delegation is not required for every change.
- Verify frontend behavior with focused unit tests, TypeScript checks, linting, builds, and API-level checks where applicable. Do **not** use Playwright or other browser automation unless the user explicitly requests it.
- When an agent stalls, cancel or replace only that task and continue with the smallest unblocked verification or implementation step.
