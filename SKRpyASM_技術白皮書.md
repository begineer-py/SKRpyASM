...
13. [CVE Intelligence 情報系統](#十三cve-intelligence-情報系統)
14. [附錄：構建與部署指南](#十四附錄構建與部署指南)
...
| 安全工具      | Nmap, Subfinder, Amass, dnsx, httpx, Nuclei, cdncheck, GAU, Katana, ffuf |
| 反機器人繞過  | FlareSolverr                                                     |
| AI 模型供應商 | OpenAI, Anthropic (Claude), Google (Gemini), Mistral, Groq, Ollama, Together |
...
Layer 1: apps.ai_assistant（框架層）
  AIAssistant 抽象基類
    ├── as_graph() → LangGraph 狀態機（setup→history→context_check→retriever→agent→prepare_tool_runs→execute_tools→compress_tool_outputs→respond）
...
Layer 3: AutomationAgent（執行層）
     AutomationAgent(AIAssistant, DBToolsMixin, ScannerToolsMixin, SpawnAgentsMixin)
     10 個 Mixin 組合出超過 60 個 @method_tool
     迭代執行 CONTEXT → CHECK DISPATCHES → PLAN → EXECUTE or DELEGATE → RECORD → SYNTHESIZE → DECIDE 迴圈

### 2.1.1 多智能體協作模式當前實現與潛在改進

當前實現：
- 透過 `SpawnAgentsMixin` 提供 `spawn_recon_agent()`, `spawn_post_exploit_agent()`, `spawn_reporting_agent()` 三類專門子代理
- 子代理以背景 Celery 任務執行，完成後透過 `notify_caller_agent()` 回報結果
- 父代理可透過 `query_dispatched_agents()` 檢查子代理狀態
- 所有代理共享 Overview 模型中的 `knowledge` 和 `summary` 欄位進行知識傳遞
- 透過自動 ID 注入機制防止 LLM 幻覺資料庫 ID

潛在改進方向：
1. **增強即時通訊機制**：目前主要依賴 `notify_caller_agent()` 和共享資料庫狀態，可考慮加入更即時的訊息佇列或事件流機制
2. **動態代理專業化**：目前代理類型預設為 Recon/PostExploit/Reporting 三種，可探索根據任務需求動態生成專業化代理的機制
3. **知識共享優化**：現有 `knowledge` JSON 欄位功能基礎，可考慮引入更結構化的知識圖譜或經驗庫，促進代理間經驗遷移
4. **錯誤恢復與容錯機制**：增強代理失敗時的自動恢復、重試策略和備用方案
5. **資源負載平衡**：基於代理能力和當前工作負載的更智能任務分配機制
6. **協作決策優化**：改進多代理間的衝突解決和共識建立機制

這些改進方向旨在提升系統的自適應性、效率和穩健性，使多智能體協作更加智能化和可靠。
...
```python
# apps/ai_assistant/helpers/assistants.py:704-667
def as_graph(self, thread_id=None, thread=None):
    ...
    workflow.add_node("context_check", context_check) # 記憶壓縮檢查
    workflow.add_node("retriever", retriever) # RAG 檢索（可選）
    workflow.add_node("agent", agent)       # LLM 推理 + 工具呼叫
    workflow.add_node("prepare_tool_runs", prepare_tool_runs) # 工具執行前準備
    workflow.add_node("execute_tools", execute_tools) # 執行工具
    workflow.add_node("compress_tool_outputs", compress_tool_outputs) # 工具輸出壓縮
    ...
    workflow.add_conditional_edges("agent", tool_selector, {
        "call_tool": "prepare_tool_runs", # 有工具呼叫 → 準備執行工具
        "continue": "respond",  # 無工具呼叫 → 回應
    })
    workflow.add_edge("prepare_tool_runs", "execute_tools")
    workflow.add_edge("execute_tools", "compress_tool_outputs") # 壓縮後送回 LLM
...
| `StepManagementMixin` | `update_overview_status`, `update_step_note`, `query_steps`, `create_verification`, `get_exhausted_attack_vectors`, `execution_node_create`              | `step_management_tools.py` |
...
| `SkillMixin`          | `search_skills`, `load_skill`, `create_or_update_skill`, `request_skill_creation`, `delete_skill`, `execute_skill_script`, `install_sandbox_dependency`  | `skill_tools.py`           |
| `SandboxMixin`        | `run_command` → Kali Docker 沙箱                                                                                                                        | `sandbox_tools.py`         |
| `CVEIntelligenceMixin` | `query_cve_by_id`, `search_cves_for_technology`, `enrich_vulnerability_with_cve`, `get_techstack_cve_report`                                             | `cve_intelligence_tools.py` |
| `SpawnAgentsMixin`    | `spawn_recon_agent`, `spawn_post_exploit_agent`, `spawn_reporting_agent`, `spawn_skill_creator`                                                          | `spawn_tools.py`           |

> 實際架構：上述 Mixin 透過 `DBToolsMixin` 複合組合（composition）後被 `AutomationAgent` 使用，而非直接繼承。`AutomationAgent` 直接繼承 `AIAssistant`, `DBToolsMixin`, `ScannerToolsMixin`, `SpawnAgentsMixin`。
...
class SkillTemplate(models.Model):
    ...
    script_content = models.TextField()         # Python/Bash 腳本（含自動注入的 I/O 驗證）
    ...
    input_schema = models.JSONField(null=True)  # Pydantic input schema
    output_schema = models.JSONField(null=True) # Pydantic output schema
    has_io_contract = models.BooleanField(default=False) # 是否已自動注入 I/O 驗證
    test_input_example = models.JSONField(null=True)     # 測試輸入範例
    is_deprecated = models.BooleanField(default=False)   # 技能廢棄標記
```

技能生命週期：

```
request_skill_creation(name, description, instructions, input_schema, output_schema)
  → LLM 生成 script_body → I/O Contract 自動注入 → script_content 寫入 DB
  → search_skills(query) → load_skill(skill_id) → execute_skill_script(skill_id, args)
    → create_or_update_skill() → 永久儲存，跨目標重用
      → delete_skill() → 清理無效技能
```

傳統的 `promote_successful_script` 與 `list_ready_for_promotion` 已廢棄（`ScriptExecution` 模型已移除），改由 `ExecutionArtifact` 查詢成功執行結果後再透過 `create_skill` / `update_skill` 建立技能。
...
### 2.6 全面可觀測性 — ExecutionGraph 四層事件模型

系統已全面採用 **ExecutionGraph** 四層事件模型取代舊版 `Step`/`StepLog`/`ScriptExecution`，所有 LLM 呼叫、工具執行、掃描派遣、Celery 任務皆自動追蹤。

#### 四層架構

```
ExecutionGraph         (運行容器，FK to Thread)
  └── ExecutionNode    (單一步驟：LLM 呼叫、工具執行、掃描任務)
       └── ExecutionEvent   (不可變、append-only 事件日誌)
            └── ExecutionArtifact  (輸出資料：掃描結果、HTTP 追蹤、備註)
```

#### ExecutionGraph

```python
# apps/core/models/execution.py
class ExecutionGraph(models.Model):
    thread = ForeignKey(Thread)           # 所屬 AI 對話線程
    assistant_id = CharField(255)         # Agent 辨識名稱
    run_id = CharField(128)               # LangChain run ID
    title = CharField(255)                # 圖表標題
    status = [RUNNING / WAITING / SUCCEEDED / FAILED / CANCELLED / BLOCKED]
    metadata = JSONField()                # 附加資訊
...
class ExecutionNode(models.Model):
    graph = ForeignKey(ExecutionGraph)    # 所屬 Graph
    parent = ForeignKey(self)             # 父節點（樹狀結構）
    name = CharField(255)                 # 節點名稱
    kind = [GRAPH / LLM / TOOL / SCANNER / TASK / SYSTEM]
    status = [PENDING / RUNNING / WAITING / SUCCEEDED / FAILED / CANCELLED / SKIPPED / BLOCKED]
    tool_call_id = CharField(255)         # 工具呼叫 ID
    external_task_id = CharField(255)     # Celery Task ID 綁定
    wait_reason = CharField(64)           # WAITING 原因
    input / output / error / metadata = JSONField()
    sequence = BigIntegerField(db_index=True)   # 單調遞增序列
...
class ExecutionEvent(models.Model):
    graph = ForeignKey(ExecutionGraph)    # 所屬 Graph
    node = ForeignKey(ExecutionNode)      # 關聯節點（可選）
    event_type = CharField(96)            # 事件類型
    status = CharField(32)                # 事件狀態
    content = TextField()                 # 可讀描述
    payload = JSONField()                 # 結構化資料
    sequence = BigIntegerField(db_index=True)  # 單調遞增
...
class ExecutionArtifact(models.Model):
    graph = ForeignKey(ExecutionGraph)    # 所屬 Graph
    node = ForeignKey(ExecutionNode)      # 關聯節點（可選）
    artifact_type = CharField(96)         # 成品類型
    name = CharField(255)
    content = TextField()                 # 文字內容
    data = JSONField()                    # 結構化資料
    content_blob = ForeignKey(ContentBlob) # 大型內容（FK 到長期儲存）
    metadata = JSONField()
    created_at = DateTimeField()
```
...
`ExecutionService`（`apps/core/services/execution.py`）提供靜態方法統一管理 Graph/Node/Event/Artifact 的完整生命週期：

| 方法 | 功能 |
|------|------|
| `start_graph()` / `complete_graph()` / `fail_graph()` | Graph 生命週期 |
| `start_node()` / `wait_node()` / `complete_node()` / `fail_node()` | Node 狀態轉換 |
| `complete_node_by_id()` / `fail_node_by_id()` | Celery callback 專用（依 ID 查找後完成/失敗） |
| `reconcile_graph_status()` | 自動從子節點狀態推導 Graph 狀態 |
| `set_node_external_task_id()` | 綁定 Celery Task ID 到 Node |
| `emit_event()` | 建立不可變事件記錄 |
| `attach_artifact()` | 關聯產出成品 |

舊版 `Step`/`StepLog`/`ScriptExecution` 模型已全部移除，相關功能完全由 ExecutionGraph 架構取代。`callback_step_id` 參數已全面改為 `execution_node_id`。

### 2.7 反幻覺防火牆

| 機制                                  | 位置                           | 說明                           |
| ------------------------------------- | ------------------------------ | ------------------------------ |
| Auto-ID Injection（雙層防禦）         | `assistants.py:279-289`        | 工具 schema 自動剝離 `overview_id`/`thread_id`/`parent_thread_id`，LLM 無法看到或幻覺這些 ID |
| Runtime ID 注入                       | `assistants.py:694-745`        | `as_graph()` 包裝工具函式，執行時從 Agent 實例屬性自動注入 ID |
...
**Auto-ID Injection 詳解**：

系統透過兩層機制防止 LLM 幻覺資料庫 ID：

1. **Schema 剝離**（`_set_method_tools()`）：將所有 `@method_tool` 的 `args_schema` 中的 `self`、`overview_id`、`thread_id`、`parent_thread_id` 欄位從 Pydantic schema 中移除。LLM 看到的工具參數中完全沒有這些 ID。

2. **Runtime 注入**（`as_graph()`）：檢查工具的原始 Python 函式簽名。若包含 `overview_id`，則建立一個閉包包裝器，在呼叫時強制注入已知的 `overview_id`（來自 `_agent_overview_id` 或 `thread.bound_overview_id`）。

工具的 `_current_invoke_thread_id`、`_caller_thread_id`、`_current_execution_graph_id` 等關鍵屬性在 `invoke()` 進入點自動解析，Agent 執行過程中完全不需要手動傳遞這些 ID。
...
| `execution_node_id` 回呼注入   | `scanner_tools.py` — 掃描完成自動回呼對應 ExecutionNode         |
| ExecutionGraph 四層追蹤       | `apps/core/models/execution.py` — Graph→Node→Event→Artifact     |
| 批量資產處理                  | Seed → Subdomain → IP → URL 全自動擴散探索                      |
| Prefork Worker 池 + late ack | `-P prefork -c 8`，`CELERY_TASK_ACKS_LATE=true` — 任務崩潰自動重新派送 |
...
Agent 的核心決策模式為 **CONTEXT-CHECK-PLAN-EXECUTE-RECORD-SYNTHESIZE-DECIDE**：基於資料庫「已經知道的」推導出「想知道的」，據此規劃下一個行動步奏。此決策模式是多智能體協作的核心，決定何時自行執行任務（EXECUTE）以及何時將任務委派給專門子代理（DELEGATE）。

```python
# apps/auto/assistants/planning_agent.py:226-236 (operational_loop)
LOOP ITERATION:
1. CONTEXT:  取得 Target Context，了解當前資產狀態
2. CHECK DISPATCHES: 檢查是否有未完成的非同步派發任務
3. PLAN:     根據情報規劃下一步具體行動
4. EXECUTE or DELEGATE: 執行同步工具（如 run_command）或非同步派發（如 run_nuclei）
5. RECORD:   記錄觀察結果（write_recon_note + update_overview_status）
6. SYNTHESIZE: 匯總當前知識，更新認知
7. DECIDE:   繼續迭代 or 通知上層 or 任務完成
```

| 階段                     | Agent 思維                                                                            | 對應工具/DB                                                                            | 實際範例                                                                                                      |
| ------------------------ | ------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| **CONTEXT**              | 資料庫已有什麼情報？哪些資產已掃過？哪些攻擊已嘗試過且失敗？                          | `get_target_context`, `get_url_intelligence`, `get_exhausted_attack_vectors`           | 查到 URL id=5 的 tech_stack = "Django 4.2"、`is_vuln_scanned = False`、已嘗試過 2 個 SQLi payload 但被 WAF 擋 |
| **CHECK DISPATCHES**     | 是否有非同步掃描尚未回呼？需要等待嗎？                                                | `get_target_context`（檢查 status/external_task_id）                                   | 確認上一輪派發的 Nuclei 掃描已完成                                                                            |
| **PLAN/EXECUTE**         | 有哪些攻擊面尚未探索？該目標是否有已知漏洞的關聯 CVE？選擇最適當的工具執行            | `check_scanned_*`, `search_skills`, `run_nuclei_*`, `run_command`                      | 執行 `run_nuclei_url(url_id=5, template="cves/2024/CVE-2024-...")`                                            |
| **RECORD/SYNTHESIZE**    | 記錄發現結果、更新知識庫、更新風險分數                                                  | `write_recon_note`, `update_overview_status(new_knowledge=...)`                        | 將帳密與 session 存入 Overview.knowledge                                                                      |
| **DECIDE**               | 繼續迭代下一目標 / 卡住向上層請教 / 任務完成通知                                         | `notify_caller_agent`, `escalate_to_orchestrator`                                      | 發現新的攻擊面 → 回到 CONTEXT                                                                                 |
...
### 4.2 ExecutionNode 樹狀結構

```python
# apps/core/models/execution.py
class ExecutionNode(models.Model):
    graph = ForeignKey(ExecutionGraph)     # 所屬執行圖
    parent = ForeignKey("self")            # 父節點（支援樹狀攻擊鏈）
    name = CharField(255)                  # 節點名稱
    kind = [GRAPH / LLM / TOOL / SCANNER / TASK / SYSTEM]
    status = [PENDING / RUNNING / WAITING / SUCCEEDED / FAILED / CANCELLED / SKIPPED / BLOCKED]
    tool_call_id = CharField(255)          # 工具呼叫 ID
    external_task_id = CharField(255)      # Celery Task ID
    wait_reason = CharField(64)            # WAITING 原因
    input / output / error / metadata = JSONField()
    sequence = BigIntegerField(db_index=True)
```

### 4.3 非同步掃描回呼鏈（ExecutionService）

```python
# apps/core/services/execution.py（透過 ScannerToolsMixin 調用）
_dispatch_scanner(overview_id, tool_name, endpoint, payload):
  1. node = ExecutionService.start_node(graph, name=tool_name, kind="SCANNER")
  2. ExecutionService.wait_node(node, wait_reason="awaiting_async_result")
  3. payload['execution_node_id'] = node.id  # 自動注入執行節點 ID
  4. POST → 內部掃描 API
  5. 掃描完成 → Celery callback → ExecutionService.complete_node_by_id(node.id, output)
```
...
| **L5: 利用與橫向** | 實際利用、提權、橫向移動          | `run_command`（Kali 沙箱）, `execute_skill_script`         | CommandExecution, ExecutionEvent |

### 4.8 持續滲透 vs 傳統一次性掃描

| 面向     | 傳統一次性掃描         | SKRpyASM 持續滲透                               |
| -------- | ---------------------- | ----------------------------------------------- |
| 執行方式 | 依序跑完整個 pipeline  | CONTEXT→CHECK→PLAN→EXECUTE→RECORD→SYNTHESIZE→DECIDE 迭代循環          |
...
```python
# scanner_tools.py（透過 ExecutionService）
_dispatch_scanner(overview_id, tool_name, endpoint, payload):
    1. node = ExecutionService.start_node(graph, name=tool_name, kind="SCANNER")
    2. ExecutionService.wait_node(node, wait_reason="awaiting_async_result")
    3. payload['execution_node_id'] = node.id  # 自動注入節點 ID
    4. POST → 內部掃描 API
    5. 若 API 回傳 4xx → ExecutionService.fail_node(node.id, error)
    6. 回傳 → Agent 繼續 THINK 下一目標
    7. 掃描完成 → Celery callback → ExecutionService.complete_node_by_id(node.id)
```

這保證了：

- Agent 不會因為等待非同步掃描結果而停滯
- Watchdog 作為最終防線，逾時 30 分鐘自動標記 ExecutionNode FAILED
- 每個掃描都有 ExecutionGraph → Event → Artifact 完整追蹤
...
### 4.9.10 非同步呼叫鏈：ExecutionGraph 驅動的回呼機制

外部服務（FlareSolverr）透過 Celery 非同步執行，並經由 `execution_node_id` 機制回呼 `ExecutionService` 完成節點。整個鏈路由三層組成，舊版 `callback_step_id` / `Step` / `@with_auto_callback` 已全面由 `ExecutionGraph` 取代。

#### 架構總覽

```
AI Agent (LangChain) ──→ ① _dispatch_scanner ──→ ② Ninja API ──→ ③ Celery Task ──→ ExecutionService
                                │                    │               │
                                │ ExecutionNode(     │ async def     │ perform_scan   │ complete_node_
                                │   kind=SCANNER,    │ .delay(       │                │ by_id(node.id)
                                │   status=WAITING)  │ execution_    │                │ → node SUCCEEDED
                                │ execution_node_id  │ node_id=…)    │                │ → emit_event()
                                │ = node.id inject   │               │                │ → attach_artifact()
```

#### ① Agent 工具層：`_dispatch_scanner`

所有需要非同步執行的掃描工具統一透過 `ScannerToolsMixin._dispatch_scanner()` 派遣：

```python
def _dispatch_scanner(self, overview_id, tool_name, endpoint, payload, description=""):
    node = ExecutionService.start_node(
        graph, name=tool_name, kind="SCANNER")
    node = ExecutionService.wait_node(
        node, wait_reason="awaiting_async_result")
    payload['execution_node_id'] = node.id    # ← 自動注入節點 ID
    resp = requests.post(url, json=payload)    # POST 到內部 Ninja API
    self.emit_thread_event("scanner_dispatched", tool_name=tool_name,
                           status="started", payload={"node_id": node.id})
```

| 參數                | 說明                                                  |
| ------------------- | ----------------------------------------------------- |
| `overview_id`       | 所屬 Overview（auto-injected）                        |
| `tool_name`         | 工具辨識名稱                                          |
| `endpoint`          | 內部 API 路徑（如 `/api/flaresolverr/start_scanner`） |
| `payload`           | API payload，會自動注入 `execution_node_id`           |
| `node.id`           | 整個 callback 鏈的核心識別碼                          |
...
async def start_crawl(request, trigger_data: FlaresolverrTriggerSchema):
    ...
    perform_scan_for_url.delay(                         # ← 非同步派遣
        ...
        execution_node_id=trigger_data.execution_node_id, # ← 傳遞給 Celery
    )
...
async def send_request(request, payload: FlaresolverrSingleRequestSchema):
    perform_flaresolverr_request.delay(
        ...
        execution_node_id=payload.execution_node_id,    # ← callback 鏈
    )
    return {"success": True, "message": "Request dispatched"}
```

#### ③ Celery Task 層：實際執行 + ExecutionService callback

所有外部呼叫皆包裝為 Celery `@shared_task`，任務完成後透過 `ExecutionService` 更新節點狀態：

```python
@shared_task(bind=True, name="flaresolverr.tasks.perform_scan_for_url")
def perform_scan_for_url(self, url, target_id, execution_node_id=None, ...):
    with ScannerLifecycle(scan_record, logger) as lc:
        result = ReconOrchestrator.run(url, target_id)
        lc.set_output(result.raw_data)
    # 完成後透過 ExecutionService 回呼
    ExecutionService.complete_node_by_id(
        node_id=execution_node_id,
        output={"urls_found": result.url_ids, "forms": result.form_count})
    ExecutionService.emit_event(graph, "scanner_completed",
                                content=f"Scan complete: {result.url_ids}")
```
...
| ~~FlareProxyGo~~ | `ghcr.io/kljensen/flareproxygo:latest`     | `8192` (API) / `1337` (Proxy) | Go Proxy 層，包裝 FlareSolverr，提供 SOCKS5 代理入口 | 未整合至後端，僅 Docker 容器運行 |
...
─────┼───────────────────────┼──────────────────────────────────────
 ...
 T+0 │ _dispatch_scanner     │ ExecutionService.start_node(kind=SCANNER)
 T+0 │                       │ ExecutionService.wait_node(reason="awaiting_async")
 T+0 │                       │ POST /api/flaresolverr/start_scanner
 T+0 │                       │   payload: {target_id: 42, execution_node_id: 101}
 ...
 T+3 │ ExecutionService      │ complete_node_by_id(101, output={...})
 T+3 │                       │ Node(101) → SUCCEEDED
 T+3 │                       │ emit_event("scanner_completed") → Event 記錄
 T+3 │                       │ create_message() 注入系統訊息到 Thread
 T+3 │ AI Agent (收到訊息)   │ 繼續 THINK → 分析新 URL
```

**ExecutionNode 狀態轉換對照**

| 階段                | ExecutionNode 狀態           | 所在組件                           |
| ------------------- | ---------------------------- | ---------------------------------- |
| Agent 決定掃描      | `WAITING`                    | `_dispatch_scanner`（同步）        |
| Celery 開始執行     | （不變，維持 WAITING）       | Celery Worker                      |
| Celery 執行完畢     | `SUCCEEDED` + `completed_at` | `ExecutionService.complete_node()` |
| 超時未完成          | `FAILED`                     | Watchdog 每 5 分鐘偵測             |
| Watchdog 發救援訊息 | —                            | `watchdog.py` → `create_message()` |

**FlareSolverr Session 重用機制**

`session_key` 讓 AI 在同一個 Overview 內重複使用同一個瀏覽器 Session：
...
#### 4.10.5 可觀測性：ExecutionGraph 作為規劃的具體化

冷啟動流程中建立的 **第一個 ExecutionGraph** 代表「Agent 被喚醒投入工作」，後續每個工具呼叫都會自動記錄為 ExecutionNode：

| ExecutionNode 建立時機             | 建立者                                                    | kind              | 狀態起點 |
| --------------------------------- | --------------------------------------------------------- | ----------------- | -------- |
| auto_execute_plan 啟動時          | `auto_execute_plan` → `ExecutionService.start_graph()`    | `GRAPH`           | RUNNING  |
| Agent invoke 開始                 | `as_graph()` → `ExecutionService.start_node()`            | `LLM`             | RUNNING  |
| Agent 調用掃描工具時              | `_dispatch_scanner` + `ExecutionService.start_node()`     | `SCANNER`         | WAITING  |
| Agent 調用工具時                  | `compress_tool_outputs` → `ExecutionService.start_node()` | `TOOL`            | RUNNING  |
| Agent 手動呼叫工具                | `StepManagementMixin` + `ExecutionService`                | `TASK`            | PENDING  |

所有 ExecutionNode 形成樹狀攻擊鏈（`parent` FK），從第一個 `LLM` 節點往下展開：

```
auto_execute_plan: Graph#1 (assistant_id="automation_agent", RUNNING)
├── ExecutionNode#1 (LLM, RUNNING) — Agent THINK 迭代
│   ├── ExecutionNode#2 (SCANNER, WAITING) ← _dispatch_scanner → execution_node_id
│   │   └── 完成 → Node#2 SUCCEEDED, Agent 收到回呼
│   ├── ExecutionNode#3 (TOOL, RUNNING) — Agent 手動工具呼叫
│   │   └── 完成 → Node#3 SUCCEEDED, ExecutionArtifact 記錄結果
│   └── ExecutionNode#4 (LLM, RUNNING) — 下一輪 THINK
└── Graph#1 SUCCEEDED — ExecutionGraph 完成
```
...
```python
# apps/core/models/analyze/Verification.py
...
class Overview(models.Model):
    ...
    status = [PLANNING / EXECUTING / STALLED / COMPLETED / NEEDS_GUIDANCE]
    risk_score = PositiveSmallIntegerField(0-100)
...
`NEEDS_GUIDANCE` 狀態表示 Agent 已向上層 Orchestrator 請求戰略指導（透過 `escalate_to_orchestrator()` 工具）。`auto_execute_plan` 排程任務會自動偵測此狀態並調用 `_handle_guidance_request()`：讀取 `knowledge._escalation` 中的問題 → LLM 生成戰略建議 → 回寫 `plan` → 恢復 `EXECUTING`。
...
所有執行追蹤已由 `ExecutionGraph` → `ExecutionNode` → `ExecutionEvent` → `ExecutionArtifact` 四層模型取代舊版 `Step`/`StepLog`/`ScriptExecution` 模型。`discovery_step`（FK to Step）等欄位已隨 Step 模型移除而刪除。
...
└──────────────────────────────────────────────────────────────────┘
  ...
  ┌──────────────┐  ┌──────────────┐  ┌────────────────┐
   ...
   │ install_sand- │  │ Node→FAILED  │  │ orchestrator() │
   │ box_dependency│  │ Event記錄    │  │                │
  └──────┬───────┘  └──────┬───────┘  └───────┬────────┘
         │                 │                   │
         └──────┬──────────┘                   │
                │                              │
...
│                  前端 React (Vite 7)                  │
│  ┌─────────────────────────────────────────────────┐ │
│  │  自訂 GraphQL Hooks (useHasuraQuery/Subscription)│ │
│  │  ├─ HTTP POST → useHasuraQuery (查詢/變更)      │ │
│  │  └─ WebSocket → useHasuraSubscription (訂閱)    │ │
│  │     (graphql-ws ^6.0.8, keepAlive 30s)          │ │
│  └─────────────────────────────────────────────────┘ │
│              │                    ▲                    │
│         HTTP  │             WebSocket                  │
│         POST  │             Subscription               │
...
| WebSocket 端點 | `ws://127.0.0.1:8085/v1/graphql`              | `frontend/src/hooks/useHasuraSubscription.ts`        |
...
| 客戶端                  | 檔案                                          | 用途                                             | 傳輸層                                                                    |
| ----------------------- | --------------------------------------------- | ------------------------------------------------ | ------------------------------------------------------------------------- |
| 自訂 Hooks             | `frontend/src/hooks/useHasuraQuery.ts`、`useHasuraSubscription.ts` | 輕量 GraphQL 客戶端，直接使用 `graphql-ws` | HTTP POST + WebSocket（`graphql-ws`），keepAlive 30s，Infinite Retry |
...
**注意：Apollo Client（`@apollo/client`）雖安裝但未被實際使用。前端 GraphQL 存取採用自訂 hooks：**

- `useHasuraQuery` → 使用 `fetch` 發送 HTTP POST 到 Hasura GraphQL endpoint，附帶 `x-hasura-admin-secret`
- `useHasuraSubscription` → 使用 `graphql-ws` `createClient` 建立 WebSocket 連線，支援 30s keepAlive 與 Infinite Retry
...
> ⚠️ **2026-06 架構更新**：`ScriptExecution` 模型已移除，`promote_successful_script` 與 `list_ready_for_promotion` 工具已廢棄。所有執行記錄現由 `ExecutionGraph` → `ExecutionNode` → `ExecutionEvent` → `ExecutionArtifact` 四層模型取代。技能建立應使用 `request_skill_creation` 替代，查詢執行結果應查詢 `ExecutionArtifact`。
...
SkillTemplate（已驗證的持久技能）
    ...
    │
ExecutionArtifact（所有技能執行記錄，FK to ExecutionNode）
    ↑
    ├── 臨時腳本：儲存在 Artifact.script_content
    ├── 已驗證腳本：可升級為 SkillTemplate
    ├── 失敗腳本：記錄失敗原因供診斷
    └── 關聯 ExecutionEvent：完整審計鏈
```
...
- **緩存**：useHasuraQuery 使用 React Query / 手動狀態管理快取結果
...
4. **前端緩存**：透過 React 狀態管理或 useMemo 緩存 Overview 和執行數據
...
/api/core/executions                  — ExecutionGraph 執行清單（過濾：target_id/thread_id/status）
...
if vuln_ids:
    enrich_vulnerabilities_batch.delay(list(vuln_ids))
```
