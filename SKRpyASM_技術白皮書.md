...
### 4.1 AutomationAgent 迭代攻防迴圈

```python
# apps/auto/assistants/planning_agent.py:44-56
LOOP ITERATION:
1. THINK:  分析當前情報，選擇最有希望的攻擊目標
2. QUERY:  get_url_intelligence(url_id) → Forms/Endpoints/TechStack/Findings/Vulnerabilities
3. ACT:    執行具體動作（curl/sqlmap/nuclei/sandbox command）
4. NOTE:   write_recon_note(overview_id, title, content) → 立即保存觀察結果
5. REPEAT: 根據發現選擇下一個目標
```
...
*本白皮書基於 SKRpyASM 原始碼分析生成，反映截至 2026-05-17 的系統架構與設計。*
