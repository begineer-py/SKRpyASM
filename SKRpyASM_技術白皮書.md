...
Layer 1: apps.ai_assistant（框架層）
  AIAssistant 抽象基類
    ├── as_graph() → LangGraph 狀態機（setup→history→context_check→retriever→agent→tools→compress_tool_outputs→respond）
...
def as_graph(self, thread_id=None, thread=None):
    ...
    workflow.add_edge("tools", "compress_tool_outputs")  # 壓縮後送回 LLM
...
外部服務（FlareSolverr / FlareProxyGo）透過 Celery 非同步執行，並經由 `execution_node_id` 機制回呼 `ExecutionService` 完成節點。整個鏈路由三層組成，舊版 `callback_step_id` / `Step` / `@with_auto_callback` 已全面由 `ExecutionGraph` 取代。
